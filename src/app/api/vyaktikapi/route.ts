...
export async function GET() {
  try {
    ...
    schemes.scheme_name AS yojna_name
FROM vaykatigatdb
...
LEFT JOIN schemes ON vaykatigatdb.scheme_name = schemes.scheme_id
  ...
  try {
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO vaykatigatdb (
        ...
        niymitaarogya, rationcard_no, rationcardtype, contact_no, taluka_id, village_id, gp_id, krushisanmanyojna, collectionofdata
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        body.castdata,
