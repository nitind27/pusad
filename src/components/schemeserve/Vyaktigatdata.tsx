...
const Vyaktigatdata: React.FC<Props> = ({
    ...
    const handleSave = async () => {
        ...
        try {
            const transformedData: Record<string, string | number> = {
                ...
                krushisanmanyojna: formData.krushisanmanyojna,
                gp_id: formData.gp_id,
                status: 'Active', // ✅ Optional, add if backend requires it
                collectionofdata: JSON.stringify(formData),
            };
    ...
    const handleEdit = (item: BhautikDataall) => {
        ...
        setFormData({
            ...
            gp_id: item.gp_id || "",
            krushisanmanyojna: item.krushisanmanyojna || "",
        });
    };
                            ...
                            <div className="bg-gray-100 rounded-lg shadow p-4 flex-1">
                                ...
                                <div className="flex space-x-3 mt-1">
                                    <label className="inline-flex items-center">
                                        <input
                                            type="radio"
                                            name="krushisanmanyojna"
                                            value="yes"
                                            checked={formData.krushisanmanyojna === 'yes'}
                                            onChange={e => handleChange('krushisanmanyojna', e.target.value)}
                                        />
                                        <span className="ml-1 text-xs text-gray-700">होय</span>
                                    </label>
                                    <label className="inline-flex items-center">
                                        <input
                                            ...
                                            name="krushisanmanyojna"
                                            value="no"
                                            checked={formData.krushisanmanyojna === 'no'}
                                            onChange={e => handleChange('krushisanmanyojna', e.target.value)}
                                        />
                                        <span className="ml-1 text-xs text-gray-700">नाही</span>
                                    </label>
